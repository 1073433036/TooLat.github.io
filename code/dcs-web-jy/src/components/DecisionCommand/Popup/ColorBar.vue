<template>
  <main id="ColorBar" class="ns" v-if="colorOptions.length">
    ...
    <div class="colorbar-maximize" v-else>
      ...
      <div class="color-wrapper">
        ...
        <ul :style="{maxHeight: defaultHeight+'px'}">
          <li v-for="(item, index) in colorArray" :key="index" :style="{height: item.rowHeight}">
      ...
      <div class="color-options" v-show="colorOptionsShow">
        <ul>
          <li v-for="(opt, index) in colorTypeOptions" :key="index" @click="selectColor(opt)">{{ opt.label }}</li>
        </ul>
      </div>
    </div>
  </main>
...
<script lang='ts'>
  ...
  import ColorTableConf from '@/config/ColorTableConf'

  @Component
  export default class ColorBar extends Vue {
    @Getter('decisionStore/isColorDelete_global') colorDeleteFlag
    @Getter('decisionStore/isColorAdd_global') colorAddFlag
    @Getter('decisionStore/colorTypeData_global') colorTypeData
    @Getter('decisionStore/deleteColorType_global') deleteColorType
    colorFlag: any = {}
    colorData: any = {}
    colorArray: any[] = []
    colorOptions: any[] = []
    ...
    defaultRowHeight: number = 20
    rowHeight: string = ''
    colorOptionsShow: boolean = false
    isMinimize: boolean = false
    modelColorConfig: any = {
      torrent: [
    ...
    changeColorTable(type) {
      ...
      this.colorUnit = this.elementUnit[currentColorData.type] || '';
    }
    ...
    async addColorBar() {
      ...
      if(this.modelColorConfig.hasOwnProperty(type)) {
        ...
        if (isTideModel) {
          try {
            response = await axios({ url: reqUrl, adapter: jsonp })
          }
          catch(err) {
            throw 'failed to get colorbar data';
          }
        } else {
          response = ColorTableConf[flag]
        }
        ...
        if(isTideModel) {
          ...
          let data = response[0].ColorTable;
    ...
    computeColorRow(data) {
      ...
      if(dataLen > maxLen) {
        ...
        const maxStrLen = Math.floor(defaultHeight/12) - 5;  //一个字节12px, -5留出5个字节作为间隔
    ...
    @Watch('colorAddFlag')
    onColorAddFlagChanged (val: any, oldVal: any) {
      this.addColorBar().then(() => { console.log('success to add color table') })
    }

    @Watch('colorDeleteFlag')
    oncolorDeleteFlagChanged (val: any, oldVal: any) {
      this.deleteColorBar()
    }
  }
</script>
...
#ColorBar {
  ...
  .colorbar-maximize {
    ...
    .color-element {
      ...
      vertical-align: top;
      font-size: 0;
      border-bottom: 1px solid rgba(255, 255, 255, .1); /*no*/
      em {
        width: 10px; /*no*/
        height: 100%;
        position: relative;
