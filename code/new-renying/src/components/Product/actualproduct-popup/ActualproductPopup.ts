...
export default class ActualproductPopup extends Vue {
  ...
  realType: any = {
    ...
    water: { show: false, cname: 'GPS水汽', unit: '' ,classname: 'water'},
  }
  date: Date = null
  hour: number = null
  minute: number = null
  ...
  addReal(type, key, data) {
    ...
    for (let i in data) {
      let opt = data[i]
      if (!opt.elems || opt.elems[key] === undefined) continue
  ...
  addWind(type, data) {
    ...
    let markers = new L.MarkerClusterGroup(), angleMarkers = new L.MarkerClusterGroup()
    for (let i in data) {
      let opt = data[i]
