...
export default class WindRadarDrawer {
  constructor(drawType: 'single' | 'multiple', canvasId: string, rulerCanvasId: string, dateTimeCanvasId: string) {
    this.drawType = drawType
    this.dataCanvasId = canvasId
    this.rulerCanvasId = rulerCanvasId
    this.dateTimeCanvasId = dateTimeCanvasId
  }
  singleRadarData: any = null
  multipleRadarData: any = null
  drawType: 'single' | 'multiple' = 'single'
  dataCanvasId: string = ''
  rulerCanvasId: string = ''
  dateTimeCanvasId: string = ''
  rectHeight = 1550  //!!! 600 -> 550 -> 655(Vue)
  rectWidth = 640 //!!! 740
  ...
  private async drawSingleRadar(datetime, addr) {
    let canvas = <HTMLCanvasElement>document.getElementById(this.dataCanvasId)
    let ctx: any = canvas.getContext('2d')

    let dateTimeCanvas = <HTMLCanvasElement>document.getElementById(this.dateTimeCanvasId)
    let dateTimeCtx: any = dateTimeCanvas.getContext('2d')
    dateTimeCtx.clearRect(0, 0, canvas.width, canvas.height)
    dateTimeCtx.save()
    ...
    for (let i = 0; i < this.singleRadarData.length; i++) {
      ...
      //绘制时间
      this.drawDateTimeText(x, this.singleRadarData[i].crttime, dateTimeCtx);
    }
  ...
  private drawExample() {
    let canvas = <HTMLCanvasElement>document.getElementById(this.rulerCanvasId)
    let ctx: any = canvas.getContext('2d')
    ctx.clearRect(0, 0, canvas.width, canvas.height)
    ctx.save()
    ...
    for (let i in this.imgHolder) {
      ...
      ctx.drawImage(img, 10, 60 + num * 20, img.width * 0.7, img.height * 0.5)
      ctx.fillText(text, 35, 80 + num * 20)
    }
    ctx.fillStyle = '#0a8789'
    //!!! 825 180
    ctx.fillText('单位:米/秒', 15, 80 + this.imgHolder.length * 20)
    ctx.restore()
  }

  private drawDateTimeText(x: number, timeScamp: string, ctx: CanvasRenderingContext2D) {
    ctx.fillText(moment(timeScamp).format('HH:mm'), x-12, 10)
    ctx.restore()
  }
