  ...
  <main id="user-management">
    <header class="main-header cf">
      ...
      <section class="option-wraper cf">
        <div @click="managementType = 'users'" for="main-cate-control"
             :style="{fontWeight: isUserManage ? 'bold' : 'normal'}"
             v-if="isRoot">
          用户管理
        </div>
        <div @click="managementType = 'info'" for="main-cate-control"
             :style="{fontWeight: !isUserManage ? 'bold' : 'normal'}">
          信息管理
        </div>
      ...
      <aside>
        <!--<section class="search-wraper cf">
          <em></em>
          <input placeholder="请输入搜索内容" for="search" />
        </section>-->
    ...
    <section class="content-wraper" id="user-manage" v-show="isUserManage">
      <header class="content-header">
        <aside>
          <ul>
            <li style="color: #1eba90 ; border-color: #1eba90 ;" @click="openUserOpratePopup('register')">
              <span>新增</span>
              <em></em>
            </li>
          </ul>
      ...
      <user-adding :userOpratePopup="userOpratePopup" :userOprateType="userOprateType"
...
import poiInfoTable from './poiInfoTable'
import poiAddingPopup from './poiAddingPopup'
...
export default {
  data() {
    return {
      ...
      jmDcsUrl: 'http://10.149.65.54:1995/JmDcs/',
      prefixUrl: 'http://10.149.65.54:9020/taishan',
  ...
  mounted() {
    ...
    this.$http.jsonp(`${this.roaUrl}dict/geologyGD/s2/${this.currentRegion.cityId},${this.currentRegion.countyId || ''}/JSONP/`)
      .then(res => {
        this.countyInfo = res.data;
        let regionData = res.data.map(el => ({name: el.county, countyId: Number(el.countyId)}));
  ...
  methods: {
    ...
    computePoiAddingOption(data) {
      ...
      if(this.selectedInfoOption === 'disasterPoi') {
        poiParams = Object.assign({}, disasterPoiParamsConf[seledSubOpt]);
      } else {
        poiParams = Object.assign({}, basePoiParamsConf[this.selectedInfoOption === 'stationInfo' ? 'station' : seledSubOpt]);
      }
    ...
    async getPoiData(type, subType, countyId) {
      ...
      let poiData = await geoPoi.getPois(type === 'stationInfo' ? 'station' : subType, curRegion.cityId, countyId || curRegion.countyId);
    ...
    deleteUser(data, index) {
      MessageBox.confirm('确定要删除此用户？', '提示', {
        ...
        /*this.$http.jsonp(`${this.jmDcsUrl}user/delete`, { params: { id: this.allUserData[index].id } })
          .then(res => {
            if (res.data) {
              this.allUserData.splice(index, 1);
              Message({
    ...
    openUserOpratePopup(type, updateData) {
      if (type === 'register') {
        ...
        this.userOpratePopup = 'update';
        this.userOpratePopupTitle = '更新用户信息';
      }
      this.userOpratePopup = true;
    },
    //获取用户管理的数据
    getAllUserData() {
      this.allUserData = [];
  ...
  header.main-header {
    ...
    >aside {
      ...
      section.user-info {
        ...
        padding-left: 10px;
        float: left;
        border-left: solid 1px rgba(233, 239, 245, .5);
        height: 30px;
        margin: 10px 0;
        cursor: pointer;
        .user-image {
...
<style lang="scss">
  ...
  .el-table th {
    background-color: rgb(234, 237, 230) !important;
  }
