<template>
  <main id="typh-poi-popup" v-drag="{handle: '.draggable'}">
    <header class="draggable">
      <h1>{{poiType}}预报</h1>
      <span @click="closeRainAndWindPopup">
    ...
    <section class="address-datetime-wraper cf">
      ...
      <div class="address-wraper">地址: {{poiAddress}}</div>
...
export default {
  ...
  computed: {
    ...
    poiType() {
      if(this.selectedModel !== 'rain' && this.selectedModel !== 'wind')
        return '';
      else
        return this.selectedModel === 'rain' ? '降雨' : '大风';
    },
    poiAddress() {
      return this.rainAndWindPoiInfo ? this.rainAndWindPoiInfo.name : '';
    }
  },
  watch: {
  ...
  methods: {
    ...
    async getRealTimeData() {
      let seletctedPoi = this.rainAndWindPoiInfo;
      ...
      try {
        let data = (await this.$http.jsonp(`http://10.148.83.228:9020/data/getNearestStation?&lon=${lon}&lat=${lat}&endTime=${endtime.Format('yyyy-MM-dd HH:00:00')}&startTime=${starttime.Format('yyyy-MM-dd HH:00:00')}&type=${this.poiType === '降雨' ? 'rain' : 'wind'}`)).data;
        data = JSON.parse(JSON.parse(data));
        if(!data || !data.id)
          return;
        this.stationName = `${data.id}-${data.name}`;
        let res = await this.$http.get(`http://10.148.83.228:8921/station/real/find_TimeRange?station_id=${data.id}&element=${this.poiType === '降雨' ? 'rfhour' : 'wd2df'}&starttime=${starttime.Format('yyyy-MM-dd HH:00:00')}&endtime=${endtime.Format('yyyy-MM-dd HH:00:00')}`);
        return res.data;
      }
      catch(err) {
        throw '获取最近站点实况数据失败';
    ...
    async getForecastData() {
      ...
      let key = this.selectedModel === 'rain' ? 'lspe' : 'wind_vel';
      ...
      sIndex = sIndex ? (sIndex - 1) : sIndex;

      fstData.shift();
      fstData = fstData.slice(sIndex, eIndex);
      //过滤无效值
      fstData = fstData.map(value => [999998, 9999, 99999, 999999, -999, -999.9, -9999].includes(Number(value)) ? 0 : Number(value));
      let filterArray = [];
      for(let i = 1; i < fstData.length; i++) {
        let val = fstData[i];
        if(key === 'lspe') {
          val -= fstData[i - 1];
        }
        filterArray.push(val < 0 ? 0 : Number(val.toFixed(1)));
      }
    ...
    drawChart(data, realData) {
      let poiData = this.rainAndWindPoiInfo;
      const poiType = this.poiType;
      if (!poiType)
        return;
      let seriesData = [];
      let fstSeriesData = [],
          realSeriesData = [],
          thresholds = poiData.threshold,
          fstData = data || poiData.fstData;
      ...
      if(realData) {
        ...
        realData.forEach((el, index) => {
          let datetime = new Date(el.datetime).Format('yyyy-MM-dd HH:00');
          if(datetime in transData === false)
            transData[datetime] = [];
          transData[datetime].push(el.elements[key]);
        });
        for(let i in transData) {
          let avg = transData[i].reduce((acc, val) => acc + val, 0) / transData[i].length;
          realSeriesData.push({
            y: avg <= 0 ? 0 : Number(avg.toFixed(1)),
      ...
      const poiUnit = poiType === '大风' ? 'm/s' : 'mm';
      highChart.chart(document.querySelector('.typh-poi-chart-container'),{
        ...
        yAxis: {
          min: 0,
          title: {
            text: `${poiType}(${poiUnit})`
          }
        },
        tooltip: {
          headerFormat: '<span style="font-size:10px">{point.key}</span><table>',
          pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
          `<td style="padding:0"><b>{point.y} ${poiUnit}</b></td></tr>`,
          footerFormat: '</table>',
          shared: true,
          useHTML: true
        },
