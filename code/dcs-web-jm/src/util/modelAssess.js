export class ModelAssess {
  constructor($http, regionData = null) {
    this._$http = $http;
    ...
    this._urlStore = {
      ...
      projshareUrl: 'http://10.148.83.228:2008/',           //项目公用服务路径
  ...
  getCounty() {
    ...
    const url = `${urlStore.projshareUrl}projshare/geo/find/counties/cityid?cityId=${this._regionData.cityId}`;
  ...
  getTowns() {
    const urlStore = this._urlStore;
    const url = `${urlStore.projshareUrl}projshare/geo/find/towns/cityid?cityId=${this._regionData.cityId}`;
    return this._$http.get(url);
  }
  ...
  async getLatestNcFile(ncType) {
    try {
      ...
      let data = (await this._$http.jsonp(url)).data;
  ...
  getNcInfo(modelName, fileName) {
    ...
    return new Promise((resolve, reject) => {
      $http.jsonp(url, { params }).then(response => {
  ...
  getPointJson(modelName, fileName) {
    ...
    return new Promise((resolve, reject) => {
      ...
      $http.jsonp(url, { params: this._areaComp({ modelName, fileName }) })
        .then(response => {
          let data = response.data;
          if (!data) {
            reject();
  ...
  getModelExtreme(ncInfo, options) {
    ...
    return this._$http.jsonp(url, { params: ncInfoArea });
  }
  ...
  async getFireSmlFile(params) {
    return new Promise((resolve, reject) => {
      this._$http.jsonp(this._urlStore.fireSimulate, { params })
        .then(response => {
          let data = response.data;
          if (!data || data === 'null') {
            reject();
  ...
  async getPollutionSmlFile(params) {
    return new Promise((resolve, reject) => {
      this._$http.jsonp(this._urlStore.pollutionSimulate, { params })
        .then(response => {
          let data = response.data;
          if (!data || data === 'null') {
            reject();
  ...
  getEvalTowns(ranges, ncInfo, options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      $http.jsonp(`${urlStore.baseUrl}${urlStore.townUrl}?${this._urlTraner(params)}`)
        .then(response => {
          let data = response.data;
          if (data instanceof Array === false || !data.length) {
            resolve([]);
  ...
  async getEffectedTownsByRanges(rangeArray, ncInfo, options={}) {
    ...
    let params = this._areaComp(ncInfoArea);
    try {
      let res = await this._$http.jsonp(`${baseUrl}${this._urlStore.townUrl}?${this._urlTraner(params)}`);
      return res.data;
    }
    catch(e) {
      throw 'getting effected towns data failed!';
  ...
  async getSimulateImg(type, params, ncInfo) {
    ...
    if (!ncInfo) {
      ...
      let fileUrl= await this._$http.jsonp(ncFileUrl, { params });
    ...
    let imgUrlStr = await this._$http.jsonp(contourUrl);
    ...
    return new Promise((resolve, reject) => {
      ...
      this._$http.jsonp(url)
        .then(res => {
          ...
          resolve(Object.assign({imgSrc: res.data.replace(/"/g, '')}, this._bound));
        })
    });
  }
  ...
  async getHistoryTyphEffectImg(tsid, fieldName = '', fun = '') {
    ...
    let res = await this._$http.jsonp(this._urlStore.baseUrl + this._urlStore.historyTyphEffectUrl + '?' + params)
    return {
      rect,
